...
export class SimpleOracle {
  ...
  public constructor(
    ...
    if (!Array.isArray(actionIds) || !Array.isArray(context) || !Array.isArray(actionFeatures)) {
  ...
  _updateWeights(newWeights: WeightsHash = {}): number[] {
    const combinedWeights: WeightsHash = Object.assign({}, this.getWeightsHash(), newWeights);
    ...
    for (let i = 0; i < this.features.length; i++) {
      ...
      this.weights[i + offset] = combinedWeights[feature] ?? this.weights[i + offset];
    }
  ...
  public setFeaturesAndUpdateWeights(
    ...
    this.contextActionIdInteractions = contextActionIdInteractions ?? this.contextActionIdInteractions;
    this.contextActionFeatureInteractions = contextActionFeatureInteractions ?? this.contextActionFeatureInteractions;
  ...
  _predictLogit(actionId: string, contextInputs: FeaturesHash = {}, actionInputs: FeaturesHash = {} ): number {
  ...
  predict(actionId: string, contextInputs: FeaturesHash = {}, actionInputs: FeaturesHash = {}): number {    
  ...
  fit(
    ...
    if (trainingData[this.targetLabel as keyof ITrainingData] !== undefined) {
      ...
      const pred = this._sigmoid(
        this._predictLogit(trainingData.actionId, trainingData.context, trainingData.actionFeatures)
      );
